...
Once my model was validated I ran my model utilzing `drive.py` and `model.h5` file generated by Keras.   An .h5 file easily store the weights and model configuration in a single file. Upon launching the simulator the vehicle began to accelerate to a set speed and was able to navigate the test track quite smoothly with no error.  This autonomous driving was recorded from a single camera positioned on the center of the vehicle, utilzing the trained behvioural cloning network to predict the correct steering angle for a given vehicle position in a lane.  

<p align="center">
<img width="800" height="510" src="https://github.com/silverwhere/Self-Driving-Car-Nanodegree---Udacity/blob/main/Project%204%20-%20Behavioural%20Cloning/examples/network_computed.jpg"</p>  
